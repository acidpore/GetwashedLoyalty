<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Berhasil! - Getwashed Loyalty</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gradient-to-br from-purple-50 to-pink-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        <!-- Success Card -->
        <div class="bg-white rounded-3xl shadow-2xl p-8 text-center">
            <!-- Success Icon with Animation -->
            <div class="mb-6">
                @if($hasReward)
                    <div class="text-8xl mb-4 animate-bounce">🎉</div>
                    <h1 class="text-3xl font-bold text-purple-900 mb-2">
                        SELAMAT {{ strtoupper($name) }}!
                    </h1>
                    <p class="text-lg text-purple-700 font-semibold">
                        Kamu Dapat DISKON!
                    </p>
                @else
                    <div class="text-8xl mb-4">✅</div>
                    <h1 class="text-3xl font-bold text-gray-800 mb-2">
                        Terima Kasih {{ $name }}!
                    </h1>
                    <p class="text-lg text-gray-600">
                        Poin Berhasil Ditambahkan
                    </p>
                @endif
            </div>

            <!-- Points Display -->
            <div class="mb-8 p-6 bg-gradient-to-r from-purple-100 to-pink-100 rounded-2xl">
                @if($hasReward)
                    <div class="text-5xl font-bold text-purple-900 mb-2">5/5</div>
                    <p class="text-purple-700 font-semibold">Poin Tercapai!</p>
                @else
                    <div class="text-5xl font-bold text-gray-800 mb-2">{{ $points }}/5</div>
                    <p class="text-gray-600">Poin Kamu Sekarang</p>
                @endif
            </div>

            <!-- Progress Bar -->
            @if(!$hasReward)
                <div class="mb-8">
                    <div class="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                        <div 
                            class="bg-gradient-to-r from-purple-500 to-pink-500 h-4 rounded-full transition-all duration-1000"
                            style="width: {{ ($points / 5) * 100 }}%"
                        ></div>
                    </div>
                    <p class="mt-2 text-sm text-gray-600">
                        Kurang {{ 5 - $points }} poin lagi untuk diskon!
                    </p>
                </div>
            @endif

            <!-- WhatsApp Notification -->
            <div class="mb-6 p-4 bg-green-50 border-2 border-green-200 rounded-xl">
                <div class="flex items-center justify-center gap-2 mb-2">
                    <svg class="w-6 h-6 text-green-600" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                    </svg>
                    <span class="font-semibold text-green-700">Notifikasi WhatsApp</span>
                </div>
                <p class="text-sm text-green-700">
                    Pesan konfirmasi akan dikirim ke WhatsApp Anda dalam beberapa detik
                </p>
            </div>

            <!-- Action Buttons -->
            <div class="space-y-3">
                <a href="{{ route('home') }}" class="block w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 rounded-xl transition">
                    🏠 Kembali ke Beranda
                </a>
                
                <a href="{{ route('checkin') }}" class="block w-full bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-3 rounded-xl transition">
                    ↻ Check-In Lagi
                </a>
            </div>
        </div>

        <!-- Thank You Note -->
        <p class="text-center text-sm text-gray-600 mt-6">
            Terima kasih sudah setia dengan Getwashed! 🚗✨
        </p>
    </div>
</body>
</html>
